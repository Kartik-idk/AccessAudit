...
async function run() {
    console.log("Starting Task 12 Semantic Recovery Experiment...");
...
`;
        ...
        if (attempt === 1) {
            ...
            prompt += "\nCRITICAL INSTRUCTION: For this specific test attempt, your JSON output MUST EXACTLY have: \\\"attribute\\\": \\\"placeholder\\\", \\\"value\\\": \\\"Search\\\". You must also perfectly preserve the required \\\"target\\\" object including element, file, line, and column. Do not use 'aria-label'.";
        } else {
        ...
        if (!postPatchTargetRuleExists) {
            ...
            success = true;
            finalStatus = "SEMANTIC_RECOVERY_SUCCESS";
        } else {
    ...
    fs.writeFileSync('task12_raw_logs.json', JSON.stringify(logs, null, 2));
    console.log("\nFinished Task 12. Results saved to task12_raw_logs.json");
