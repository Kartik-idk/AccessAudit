...
export function processRemediations(proposals) {
  ...
  for (const idx in proposals) {
    ...
    // --- NEW: SEMANTIC GATEKEEPER ---

    const val = proposal.value || "";
    
    // Rule A: aria-hidden on interactive elements
    if (proposal.attribute === 'aria-hidden' && val === "true" && (proposal.operation === 'ADD' || proposal.operation === 'UPDATE')) {
      ...
      const hasPositiveTabindex = opening.attributes.some(a => a.name && a.name.name === 'tabIndex' && a.value && a.value.value === "0"); // AST parses {0} differently, but keeping it simple
      
      if (interactiveTags.includes(elementName) || hasClickHandler) {
        console.error("❌ SEMANTIC_REJECT: aria-hidden=true would remove an interactive element from the accessibility tree.");
        continue;
      }
    }
    ...
    if (['alt', 'aria-label', 'title'].includes(proposal.attribute) && val.trim() === "" && (proposal.operation === 'ADD' || proposal.operation === 'UPDATE')) {
      console.error(`❌ SEMANTIC_REJECT: Empty accessible name provided for '${proposal.attribute}'.`);
      continue;
    }
    ...
    if (proposal.attribute === 'tabIndex' && (proposal.operation === 'ADD' || proposal.operation === 'UPDATE')) {
      ...
      if (tabNum > 0) {
        console.error(`❌ SEMANTIC_REJECT: Positive tabIndex (${val}) is dangerous and disrupts document flow.`);
        continue;
      }
    }
    ...
    if (proposal.operation === 'REMOVE' && ['onClick', 'onKeyDown', 'onKeyUp', 'onChange'].includes(proposal.attribute)) {
      console.error(`❌ SEMANTIC_REJECT: Removing event handler '${proposal.attribute}' destroys component logic.`);
      continue;
    }

    // Rule E: Native semantic preservation (Conservative)
    if (proposal.attribute === 'role' && proposal.operation === 'ADD') {
       if (elementName === 'nav' && val === 'navigation') {
           console.error(`❌ SEMANTIC_REJECT: Adding role='navigation' to <nav> is redundant ARIA.`);
           continue;
       }
       // We allow removing redundant role
    }
    
    // --- END SEMANTIC GATEKEEPER ---
