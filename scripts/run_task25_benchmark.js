...
async function callGeminiAPI(messages) {
    ...
    let sys = undefined;
    if (geminiMessages[0].role === 'system') {
        sys = { parts: [{ text: geminiMessages[0].parts[0].text }] };
...
async function run() {
    console.log("Starting Task 25 Benchmark with frontier model: models/gemini-3.8-flash");
...
Revise your proposal.` });
    ...
    await browser.close();
    fs.writeFileSync(targetFile, originalSource);
    fs.writeFileSync('task25_results.json', JSON.stringify(results, null, 2));
    console.log("\
Results:", results);
}
