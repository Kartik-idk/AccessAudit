...
const WORKSPACE_DIR = path.resolve('./experiments/phase2d/workspace');
const REPORTS_DIR = path.resolve('./experiments/phase2d/reports');
...
async function invokeModel(condition, axeFinding, provenance) {
    ...
    if (condition === 0) {
        systemPrompt = V0_SCHEMA_PROMPT;
    } else {
        systemPrompt = `${FEW_SHOT_DEMOS}\n\nTask:\n${V0_SCHEMA_PROMPT}`;
    }
...
async function main() {
    ...
    fs.writeFileSync(path.join(REPORTS_DIR, 'phase2d_raw_results.json'), JSON.stringify(allResults, null, 2));
